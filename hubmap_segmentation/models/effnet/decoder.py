...
class MAnetDecoder(nn.Module):
    def __init__(
        ...
        blocks = [
            MFAB(in_ch, skip_ch, out_ch, scale_factor=scf, reduction=reduction, **kwargs)
            if skip_ch > 0
            else DecoderBlock(in_ch, skip_ch, out_ch, scale_factor=scf, **kwargs)
            for in_ch, skip_ch, out_ch, scf in zip(in_channels, skip_channels, out_channels, scale_factors)
        ]
